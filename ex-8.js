...
const totalPaidByLocation = bills
  ...
  .filter((place, index, arr) => {
    return arr.indexOf(place) == index;
  });

for (let i = 0; i < totalPaidByLocation.length; i++) {
  eachLocate = bills.filter((item) => {
    return item.location == totalPaidByLocation[i];
  });
  result[totalPaidByLocation[i]] = eachLocate.reduce((acc, cur) => {
